...
public class PCloudStorageStrategyImpl implements LocalStorageStrategy {
	...
	public Collection<String> getAllContainerNames() {
		try {
			...
			return rootFolder.children().stream().map(re -> re.name()).collect(Collectors.toList());
		} catch (IOException | ApiError e) {
			throw new PCloudBlobStoreException(e);
		}
	}
	...
	public void clearContainer(String container) {
		this.pCloudContainerNameValidator.validate(container);
		try {
			final RemoteFolder folder = this.getApiClient().listFolder(this.createPath(container)).execute();
	...
	public boolean blobExists(String container, String key) {
		try {
			return this.getApiClient().listFolder(createPath(container, key)).execute().isFile();
		} catch (IOException | ApiError e) {
			if (e instanceof ApiError && PCloudError.parse((ApiError) e) == PCloudError.DIRECTORY_DOES_NOT_EXIST) {
				return false;
			}
	...
	public String putBlob(String containerName, Blob blob) throws IOException {
		...
		try {
			...
			final Call<RemoteFile> createdFile = this.getApiClient().createFile(path, name,
					ds, UploadOptions.OVERRIDE_FILE);
	...
	public BlobAccess getBlobAccess(String container, String key) {
		this.logger.warn("Retrieving access rights for PCloud blobs not supported: %s%s%s", container, getSeparator(), key);
		return BlobAccess.PRIVATE;
	}

	@Override
	public void setBlobAccess(String container, String key, BlobAccess access) {
		this.logger.warn("Setting access rights for PCloud blobs not supported: %s%s%s", container, getSeparator(), key);
	}
